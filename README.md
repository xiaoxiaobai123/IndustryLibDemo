...
> 这 4 步是实测过的：在临时分支上照着做了一个"客户C"（只看噪声，>15 判 NG），
> Linux / Windows 双平台一次编过，`BUILD_TAG` 变成 `common+A+C`，
> 流程清单变成 `common / customer_a / customer_c` —— 注意此时 `flow_customer_b.c`
> 文件就在目录里，但没开 `FLOW_B`，它就是不在二进制里。任意组合，互不干扰。
